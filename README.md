...
This file is the consolidated Promily manual: setup, concepts, usage cookbook, full public API, returned-object APIs, option types, package layout, sourcemap configuration, tests, and lifecycle behavior.
...
```lua
print(Promily.getVersion()) -- "1.0.0"
```

Promily has **7 source modules**, **33 public `Promily.*` functions**, and no runtime third-party dependencies.
...
The checked-in `sourcemap.json` shape is:

```json
{"name":"Promily","className":"ModuleScript","filePaths":["src/init.luau","default.project.json"],"children":[{"name":"adapters","className":"Folder","children":[{"name":"roblox","className":"ModuleScript","filePaths":["src/adapters/roblox.luau"]}]},{"name":"combinators","className":"Folder","children":[{"name":"collection","className":"ModuleScript","filePaths":["src/combinators/collection.luau"]},{"name":"control","className":"ModuleScript","filePaths":["src/combinators/control.luau"]}]},{"name":"core","className":"Folder","children":[{"name":"cancellation","className":"ModuleScript","filePaths":["src/core/cancellation.luau"]},{"name":"promise","className":"ModuleScript","filePaths":["src/core/promise.luau"]},{"name":"scope","className":"ModuleScript","filePaths":["src/core/scope.luau"]}]}]}
```

That maps the package root as a `ModuleScript` backed by both `src/init.luau` and `default.project.json`, with source folders/modules represented under `children`.
...
| `src/core/promise.luau` | Promise state machine, chaining, awaiting, inspection |
...
The current tests cover core chaining, rejection recovery, `finally`, cancellation cleanup, tokens, scopes, combinators, concurrency limiting, timeouts, retry, Roblox adapters, wrapping, and result conversion.
...
| Check | Result |
| --- | ---: |
| Source ModuleScripts | 7 |
| Test Luau files | 2 |
| Public `Promily.*` functions | 33 |
| Promise instance methods | 28 |
| Source lines | 3,038 |
| Test lines | 452 |
| Runtime dependencies | 0 |
| `--!strict` Luau files | 9/9 |
...
- `fromProperty()` reads once, then uses `GetPropertyChangedSignal`.
- `fromAttribute()` reads once, then uses `GetAttributeChangedSignal`.
- `fromChild()` scans the existing direct/recursive hierarchy once, then uses `ChildAdded` or `DescendantAdded`.
...
Before publishing `1.0.0`, run the included Rojo project in Roblox Studio and require that `ServerScriptService/PromilyTests/run.server.luau` completes with zero failures.
